...
<div class="col-md-4 col-lg-12 col-xl-4">
    ...
    <div class="mb-9">
        <div class="d-flex flex-stack">
            <div class="fw-bolder fs-4">{{ str_replace('_',' ', \App\Models\Task::YET_TO_STATUS) }}
                <span
                    class="fs-6 text-gray-400 ms-2">{{ $project->tasks->filter(function($task) { return $task->status == \App\Models\Task::YET_TO_STATUS; })->count() }}</span>
            </div>
            <!--begin::Menu-->
            <div>
    ...
    <!--end::Col header-->
    @foreach($project->tasks->filter(function ($item) { return $item->status == \App\Models\Task::YET_TO_STATUS;}) as $task)
        <!--begin::Card-->
        <div class="card mb-6 mb-xl-9">
            ...
            <div class="card-body">
                    ...
                    <div>
                        ...
                        <div
                            ...
                            <div class="menu-item px-3 my-1">
                                <a href="{{ route('admin.task.edit', $task->id) }}" id="taskSettings"
                ...
                <!--begin::Title-->
                <div class="mb-2">
                    <a href="javascript:void(0);" id="kt_drawer_example_dismiss_button" data-id="{{$task->id}}"
                       class="fs-4 fw-bolder mb-1 text-gray-900 text-hover-primary">{{ $task->subject }}</a>
                </div>
                <!--end::Title-->
                <!--begin::Content-->
                    ...
                    <div class="symbol-group symbol-hover my-1">
                        @foreach($task->employees as $employee)
                            <div class="symbol symbol-35px symbol-circle" data-bs-toggle="tooltip" title=""
                                 data-bs-original-title="{{ $employee->user->getFullName() }}">
                                <span
                                    class="symbol-label bg-{{ config('houmanity.settings.symbol_label_colors')[$employee->user->getNameFirstLetter()] }} text-inverse-warning fw-bolder">{{ $employee->user->getNameFirstLetter() }}</span>
                            </div>
                        @endforeach
                    </div>
                    <!--end::Users-->
    ...
    <a href="javascript:void(0);" class="btn btn-primary er w-100 fs-6 px-8 py-4" data-bs-toggle="modal"
       data-bs-target="#kt_modal_new_target">Create New Task</a>
</div>
<!--end::Col-->
